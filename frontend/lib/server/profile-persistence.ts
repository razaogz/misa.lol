...
export function sanitizeProfilePayload(raw: unknown, user: User, existing: ProfileConfig | null): ProfileConfig {
  ...
  for (const [key, allowed] of [["layout",["Default","Modern","Simplistic","Sleek","Portfolio"]],["avatarShape",["circle","rounded","square"]],["bannerShape",["rounded","square","pill"]],["buttonStyle",["glass","solid","outline"]],["profileFontScope",["all","name"]],["pageEnter",["None","Fade","Unfold","Pop"]],["backgroundEffect",["None","Snowflakes","Snow","Sakura","Rain","Fireflies"]],["usernameEffect",["None","Glow","Gradient","Shimmer","Rainbow","Fuzzy","Shuffle","Sparkle","Glitch","Pulse","Wave","Shadow", "Blue Sparkles", "Green Sparkles", "Pink Sparkles", "Red Sparkles", "White Sparkles", "Yellow Sparkles"]],["socialAlign",["left","center","right"]],["cardAlign",["left","center","right"]]] as const) { const value = String(incomingSettings[key] || ""); (config.settings as Record<string, unknown>)[key] = (allowed as readonly string[]).includes(value) ? value : (previous.settings as Record<string, unknown>)[key]; }
