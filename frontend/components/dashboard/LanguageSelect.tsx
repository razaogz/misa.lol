...
export function LanguageSelect({ compact = false }: { compact?: boolean }) {
  ...
  return <div ref={root} className={`relative ${compact ? "" : "w-full"}`}>
    ...
    {typeof document !== "undefined" && createPortal(<AnimatePresence>{open && <motion.div ref={menu} role={position.sheet ? "dialog" : undefined} aria-modal={position.sheet || undefined} aria-label={t("language.label")} onKeyDown={e => { if (e.key !== "Tab") return; if (!position.sheet) { e.preventDefault(); close(); return; } const buttons = Array.from(menu.current?.querySelectorAll<HTMLButtonElement>("button") || []); const index = buttons.indexOf(document.activeElement as HTMLButtonElement); e.preventDefault(); buttons[(index + (e.shiftKey ? -1 : 1) + buttons.length) % buttons.length]?.focus(); }} initial={{ opacity: 0, x: reduced ? 0 : -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: reduced ? 0 : -8 }} transition={{ duration: reduced ? 0 : .18 }} className="glass-floating fixed z-[200] rounded-card p-1.5" style={{ left: position.left, top: position.top, width: position.sheet ? Math.min(280, window.innerWidth - 24) : 248, maxHeight: position.height, overflowY: "auto" }} onBlur={e => { if (e.relatedTarget && !e.currentTarget.contains(e.relatedTarget as Node)) close(false); }}>
