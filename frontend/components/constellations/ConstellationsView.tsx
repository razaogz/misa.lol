        ...
        <>
          ...
          <div className={styles.workspace}>
            <section className={styles.controls}>
              ...
              {tab === "design" ? <DesignPanel group={group} owner={owner} busy={busy} memberProfile={memberProfile} profileDraft={profileDraft} onProfileDraft={setProfileDraft} onSaveProfile={async (profile) => { const result = await apply(() => constellationApi.updateProfile(group.id, profile), "Constellation design saved."); if (!result) throw new Error("Constellation design could not be saved."); }} onLocal={setWorking} onSave={(patch) => void apply(() => constellationApi.update(group.id, patch), "Constellation settings saved.")} onReset={() => void resetLayout(group, apply)} /> : null}
...
function DesignPanel({ group, owner, busy, memberProfile, profileDraft, onProfileDraft, onSaveProfile, onLocal, onSave, onReset }: { group: ConstellationGroup; owner: boolean; busy: boolean; memberProfile: ProfileConfig | null; profileDraft: ProfileConfig | null; onProfileDraft: (profile: ProfileConfig) => void; onSaveProfile: (profile: ProfileConfig) => Promise<void>; onLocal: (group: ConstellationGroup) => void; onSave: (patch: Partial<ConstellationGroup>) => void; onReset: () => void }) {
  const [slugError, setSlugError] = useState("");
  ...
  return <div className={styles.panelBody}>
    ...
    {initialDesign ? <ProfileDraftProvider initialConfig={initialDesign} draftKey={group.id} onDraftChange={onProfileDraft} onSave={onSaveProfile}><ConstellationProfileControls /></ProfileDraftProvider> : <p className={styles.muted}>Your independent Constellation design is loading.</p>}
  </div>;
}
