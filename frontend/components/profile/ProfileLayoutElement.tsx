...
export function ProfileLayoutElement({ id, children }: { id: LayoutElement; children: ReactNode }) {
  ...
  const label = id === "frame" ? "Frame" : id === "discord" ? "Discord" : id === "audio" ? "Audio" : "Widget";
  ...
  const start = (event: PointerEvent<HTMLButtonElement>, mode: "move" | "resize") => {
    ...
    drag.current = { pointer: event.pointerId, x: event.clientX, y: event.clientY, box: { ...box, width: mode === "resize" ? width : box.width, height: mode === "resize" ? ref.current.offsetHeight : box.height }, draft: box, travel: Math.max(1, id === "frame" ? parentWidth - width : parentWidth), mode, viewport };
  ...
  return <div ref={ref} data-layout-element={id} data-selected={editing && selected === id ? "true" : undefined} className={`profile-element profile-element-${id.startsWith("widget:") ? "widget" : id}`} style={elementStyle(settings, id)} onPointerDownCapture={editing ? (event) => { if ((event.target as HTMLElement).closest("[data-layout-element]") === event.currentTarget) select(id); } : undefined} onPointerDown={editing ? (event) => event.stopPropagation() : undefined}>
    {children}
    {editing && <button type="button" className="profile-move-handle" aria-label={`Move ${label}`} aria-pressed={selected === id} onPointerDown={(event) => start(event, "move")} onPointerMove={move} onPointerUp={finish} onPointerCancel={(event) => finish(event, true)} onKeyDown={(event) => {
    ...
    }}><Grip size={14} aria-hidden="true" /><span>{label}</span></button>}
    {editing && id === "frame" && selected === id && <button type="button" aria-label="Resize frame" className="profile-resize-handle" onPointerDown={(event) => start(event, "resize")} onPointerMove={move} onPointerUp={finish} onPointerCancel={(event) => finish(event, true)} onKeyDown={(event) => {
