  ...
  return <main className="mx-auto min-h-screen max-w-[1050px] px-5 py-8 sm:px-8 sm:py-11 xl:px-12">
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <Link href="/security" className="surface surface-hover flex items-center gap-4 rounded-2xl p-5"><span className="icon-glass flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-[#ff6b8a]"><LockKeyhole size={18} /></span><span className="min-w-0 flex-1"><span className="block text-sm font-medium text-zinc-200">{t("settings.openSecurity", undefined, "Open Security settings")}</span><span className="mt-1 block text-xs text-zinc-600">{t("settings.openSecurityHint", undefined, "Manage email, connected accounts, account tools, and deletion.")}</span></span><span className="text-zinc-600">→</span></Link>
      </section>
    </div>
  </main>;
}
