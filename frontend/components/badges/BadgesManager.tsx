      ...
      {!collection ? <div className="surface rounded-2xl p-10 text-center text-sm text-zinc-500">Loading your collection…</div> : <>
        ...
        <section className="mt-8">
          ...
          <div className="surface flex min-h-24 flex-wrap items-center gap-3 rounded-2xl p-4">
            {selected.length ? selected.map((id, index) => { const badge = owned.find((item) => item.id === id); return badge ? <button key={id} type="button" onClick={() => toggleFeatured(badge)} className="group relative flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/[.04]" title={`Remove ${badge.name}`}><BadgeVisual badge={badge} className="h-8 w-8" /><span className="absolute -right-1 -top-1 rounded-full bg-[#f00646] px-1.5 text-[9px] text-white">{index + 1}</span></button> : null; }) : <p className="text-sm text-zinc-600">Select an owned badge below to feature it.</p>}
          </div>
        </section>
        <section className="mt-10">
          ...
          <div className="mb-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
            <div className="relative"><Search size={15} className="absolute left-3 top-3.5 text-zinc-600" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search badges" className="h-11 w-full rounded-xl border border-white/[.08] bg-white/[.025] pl-9 pr-3 text-sm text-zinc-200 outline-none focus:border-[#f00646]/50" /></div>
            <select value={category} onChange={(event) => setCategory(event.target.value)} className="h-11 rounded-xl border border-white/[.08] bg-[#0b0b0e] px-3 text-sm text-zinc-300 outline-none"><option value="all">All categories</option>{collection.categories.map((item) => <option key={item.id} value={item.slug}>{item.name}</option>)}</select>
          </div>
